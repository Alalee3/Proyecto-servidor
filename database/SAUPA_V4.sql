CREATE TABLE `calendario_academico` (
  ...
  `nota_calendario_academico` json,
...
CREATE TABLE `bitacora_justificativo` (
...
CREATE TABLE `planificacion` (
  ...
  `notificado` TINYINT,
  `motivo_rechazo_vocero` text,
  `id_firma_coordinador` text,
  `id_firma_vocero` text,
  `id_firma_docente` int,
  `estatus` enum(1,2,3,4,5),
...
ALTER TABLE `bitacora_justificativo` ADD FOREIGN KEY (`id_evento`) REFERENCES `evento` (`id_evento`);

ALTER TABLE `bitacora_justificativo` ADD FOREIGN KEY (`id_calendario_academico`) REFERENCES `calendario_academico` (`id_calendario_academico`);
