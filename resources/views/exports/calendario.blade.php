<table>
    ...
    <thead style="font-weight: bold;">
        <tr>
            <th colspan="23" style="text-align: center; font-size: 14pt;">CALENDARIO ACADÉMICO {{ $startYear }} - {{ $endYear }}</th>
        </tr>
        <tr>
    ...
    @foreach($years as $yearLoop)
        @php
            $eventDays = $eventDaysByYear[$yearLoop] ?? [];

            // Filtrar meses de este año que solapan con la vigencia
            $mesesValidos = [];
        ...
        @foreach($mesesChunks as $chunkIndex => $chunk)
            ...
            <tr style="font-weight: bold;">
                @foreach($chunk as $m)
                    <td colspan="7" style="text-align: center; border: 0.5px solid #000; background-color: #f2f2f2;">{{ $mesesNombres[$m-1] }} {{ $yearLoop }}</td>
                    <td style="width: 20px;"></td>
                @endforeach
                @if($isFirstYear && $chunkIndex == 0)
            ...
            @for($numFila = 0; $numFila < 6; $numFila++)
                <tr>
                        ...
                        @for($col = 0; $col < 7; $col++)
                            @php
                                ...
                                if ($diaNum >= 1 && $diaNum <= $daysInMonth) {
                                    $cellDate = \Carbon\Carbon::create($yearLoop, $m, $diaNum)->startOfDay();
                                }
                    ...
                    {{-- Eventos (se listan solo al lado del primer año --}}
                    @if($isFirstYear && $eventoIndex < $totalEventos)
        ...
        {{-- Eventos restantes al terminar el primer año --}}
        @if($isFirstYear)
