        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 sm:rounded-lg">
            ...
            <div class="sogat-table-container">
                ...
                <div
                    ...
                    <x-table.search-input model="busqueda" placeholder="Buscar tipo..." debounce="300ms" />
                </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @if ($tiposEvaluacion->isNotEmpty())
                            @foreach ($tiposEvaluacion as $tipo)
                                <tr wire:key="{{ $tipo->id_tipo_evaluacion }}"
                                    ...
                                    <td class="px-4 py-4 text-right">
                                        <span class="{{ $tipo->estatus == 1 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }} 
                                                            text-xs font-medium px-2.5 py-0.5 rounded">
                                            {{ $tipo->estatus == 1 ? 'Activo' : 'Inactivo' }}
                                        </span>
                                    </td>
                                    <!-- Acciones -->
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end space-x-3">
                                            <!-- Ver -->
                                            @can('ver-tipo-evaluacion')
                                                <a href="{{ route('tipo-evaluacion/show', $tipo->id_tipo_evaluacion) }}"
                                                    wire:navigate
                                            ...
                                            @endcan
                                            <!-- Editar -->
                                            @can('editar-tipo-evaluacion')
                                                <a href="{{ route('tipo-evaluacion/update', $tipo->id_tipo_evaluacion) }}"
                                                    wire:navigate
                                            ...
                                            @can('cambiar-estatus-tipo-evaluacion')
                                                @if ($tipo->estatus == 1)
                                                    <!-- Inactivar -->
                                                    <button wire:click="confirmarInhabilitar({{ $tipo->id_tipo_evaluacion }})"
                                                ...
                                                @else
                                                    <!-- Activar -->
                                                    <button wire:click="confirmarRestaurar({{ $tipo->id_tipo_evaluacion }})"
                        ...
                        @else
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td colspan="3" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    {{ $busqueda ? 'No se encontraron tipos' : 'No hay tipos registrados' }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
                ...
                <x-table.delete-modal :show="$idInhabilitar !== null" title="Confirmar Inactivación"
                    message="¿Estás seguro de que deseas inactivar este tipo de evaluación?"
                ...
                <x-table.delete-modal :show="$idRestaurar !== null" title="Confirmar Activación"
                    message="¿Estás seguro de que deseas activar este tipo de evaluación?"
