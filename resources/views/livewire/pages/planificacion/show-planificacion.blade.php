        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                ...
                <div class="flex justify-between items-start border-b pb-4 mb-4 dark:border-gray-700">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Docente: {{ $planificacion->docente_nombre ?? '' }} {{ $planificacion->docente_apellido ?? '' }} |
                ...
                <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">Periodo Académico:</p>
                        <p class="text-gray-600 dark:text-gray-400">{{ $planificacion->nombre_lapso ?? 'N/A' }}</p>
                    </div>
                </div>
                        ...
                        @foreach ($planificacion->cortes as $corte)
                            <div x-data="{ open: false }"
                                ...
                                <button @click="open = !open"
                                    class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 flex justify-between items-center hover:bg-gray-100 dark:hover:bg-gray-600 transition">
                                    <span class="font-medium text-gray-800 dark:text-gray-200">
                                        Corte #{{ $corte->corte }}
                                        <span class="ml-2 text-xs font-normal text-gray-500 dark:text-gray-400">
                                            ({{ count($corte->contenidos ?? []) }} Contenidos)
                                        </span>
                                    </span>
                                ...
                                <div x-show="open" x-collapse class="bg-white dark:bg-gray-800 p-4 space-y-4">
                                    {{-- Contenidos del Corte --}}
                                    @if (!empty($corte->contenidos))
                                        <div>
                                            <h4 class="font-semibold text-gray-700 dark:text-gray-300 mb-2 border-b dark:border-gray-700 pb-1">
                                                Contenidos
                                            </h4>
                                            <ul class="list-disc list-inside space-y-2 text-sm text-gray-600 dark:text-gray-400">
                                                @foreach ($corte->contenidos as $contenido)
                                                    <li>
                                                        <span class="font-medium text-gray-800 dark:text-gray-200">{{ $contenido['titulo_contenido'] ?? 'Sin Título' }}</span>
                                        ...
                                        <div class="mt-4 border-t pt-4 dark:border-gray-600">
                                            @if (empty($motivosRechazoCortes[$corte->detalle_id]) && empty($mostrarMotivoRechazoCorte[$corte->detalle_id]))
                                                <button wire:click="mostrarTextareaMotivo({{ $corte->detalle_id }})"
                                                    ...
                                                    Rechazar este corte específicamente
                                                </button>
                                            @endif

                                            @if (!empty($mostrarMotivoRechazoCorte[$corte->detalle_id]) || !empty($motivosRechazoCortes[$corte->detalle_id]))
                                                <div class="space-y-2">
                                                    <x-input-label for="motivo_rechazo_{{ $corte->detalle_id }}" :value="__('Motivo de Rechazo (Corte #' . $corte->corte . ')')" />
                                                    <textarea wire:model.defer="motivosRechazoCortes.{{ $corte->detalle_id }}" id="motivo_rechazo_{{ $corte->detalle_id }}" rows="3"
                                                    ...
                                                    <div class="flex items-center space-x-2">
                                                        ...
                                                        @else
                                                            <button wire:click="eliminarMotivoRechazo({{ $corte->detalle_id }})"
                                                                ...
                                                                <svg class="w-3 h-3 mr-1" fill="none" class="w-4 h-4"
