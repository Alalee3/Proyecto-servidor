<div>
    ...
    <x-slot name="header">
        <h2
            class="font-semibold text-xl text-center {{ $pnf && $pnf->estatus != 1 ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-gray-500' }} leading-tight uppercase">
            {{ $pnf && $pnf->estatus != 1 ? __('Detalles del PNF Inactivo') : __('Detalles del PNF') }}
        </h2>
    </x-slot>
            ...
            <div class="sogat-card">
                ...
                @if ($pnf)
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                        <div>
                            <x-input-label value="Nombre del PNF:" />
                            <p class="text-gray-700 dark:text-gray-300 text-2xl font-semibold break-words overflow-hidden">{{ $pnf->nombre }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-700 dark:text-gray-300 text-2xl font-semibold">
                                {{ \Carbon\Carbon::parse($pnf->fecha_creacion)->format('d/m/Y H:i') }}
                            </p>
                        </div>
                    </div>
                @else
                    <p class="text-gray-500 dark:text-gray-400">No se ha encontrado el PNF...</p>
                @endif

                <!-- Botón Volver -->
                <div class="flex justify-end mt-6">
                    <a href="{{ route('pnf/listar') }}">
