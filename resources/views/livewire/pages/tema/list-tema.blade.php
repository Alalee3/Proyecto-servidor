        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 sm:rounded-lg">
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <div
                    ...
                    <x-table.search-input model="busqueda" placeholder="Buscar Tema..." debounce="300ms" />
                </div>
                            ...
                            @foreach ($temas as $tema)
                                <tr wire:key="{{ $tema->id_tema }}"
                                    ...
                                    <td class="px-4 py-4 text-right">
                                        <span
                                            class="{{ $tema->estatus == 1 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }} text-xs font-medium px-2.5 py-0.5 rounded">
                        ...
                        @else
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td colspan="4" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    {{ $busqueda ? 'No se encontraron temas' : 'No hay temas registrados' }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
                ...
                <x-table.delete-modal :show="$idInhabilitar !== null" title="Confirmar Inhabilitación"
                    message="¿Estás seguro de que deseas inhabilitar este tema?"
                ...
                <x-table.delete-modal :show="$idRestaurar !== null" title="Confirmar Restauración"
                    message="¿Estás seguro de que deseas restaurar este tema?" cancel-method="$set('idRestaurar', null)"
            ...
            <div
                ...
                <div>@if ($temas->isNotEmpty()) {{ $temas->links() }} @endif</div>
            </div>
        </div>
    </div>
</div>