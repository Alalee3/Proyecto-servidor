        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 sm:rounded-lg">
            ...
            <div class="sogat-table-container">
                <div
                    ...
                    <x-table.search-input model="busqueda" placeholder="Buscar por usuario..." debounce="300ms" />
                </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 font-medium text-gray-900 dark:text-white">Firma</th>
                    ...
                    <tbody>
                        @if ($firmas->isNotEmpty())
                            @foreach ($firmas as $firma)
                                <tr wire:key="{{ $firma->id_firma }}"
                                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-4 py-4 text-gray-900 dark:text-white break-words">
                                        <div class="flex items-center gap-2">
                                            <div class="w-8 h-8 rounded-full bg-sogat-blue dark:bg-blue-700 flex items-center justify-center text-white text-sm font-bold">
                                                {{ strtoupper(substr($firma->usuario->name ?? '?', 0, 1)) }}
                                            </div>
                                            <div>
                                                <span class="font-medium">{{ $firma->usuario->name ?? 'N/A' }}</span>
                                                <span class="block text-xs text-gray-500 dark:text-gray-400">C.I: {{ $firma->usuario->usu_cedula ?? 'N/A' }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        @if ($firma->foto_base64)
                                            <img src="{{ $firma->foto_base64 }}" alt="Firma de {{ $firma->usuario->name ?? '' }}"
                                                class="h-12 w-auto object-contain border border-gray-200 dark:border-gray-600 rounded bg-white dark:bg-gray-900"
                                                style="image-rendering: pixelated;">
                                        @else
                                            <span class="text-gray-400 dark:text-gray-500 italic text-xs">Sin firma</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-4 text-right">
                                        <span class="{{ $firma->estatus == 1 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }} text-xs font-medium px-2.5 py-0.5 rounded">
                                            {{ $firma->estatus == 1 ? 'Activo' : 'Inactivo' }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end space-x-3">
                                            <a href="{{ route('firma/show', $firma->id_firma) }}" wire:navigate
                                            ...
                                            <a href="{{ route('firma/update', $firma->id_firma) }}" wire:navigate
                                            ...
                                            @if ($firma->estatus == 1)
                                                <button wire:click="confirmarInhabilitar({{ $firma->id_firma }})"
                                            ...
                                            @else
                                                <button wire:click="confirmarRestaurar({{ $firma->id_firma }})"
                        ...
                        @else
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td colspan="4" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    {{ $busqueda ? 'No se encontraron firmas' : 'No hay firmas registradas' }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
                ...
                <x-table.delete-modal :show="$idInhabilitar !== null" title="Confirmar Inactivación"
                    message="¿Estás seguro de que deseas inactivar esta firma?"
                ...
                <x-table.delete-modal :show="$idRestaurar !== null" title="Confirmar Activación"
                    message="¿Estás seguro de que deseas activar esta firma?"
