        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 sm:rounded-lg">
            ...
            <div class="sogat-table-container">
                ...
                <div
                    ...
                    <x-table.search-input model="busqueda" placeholder="Buscar evaluación..." debounce="300ms" />
                </div>
                    ...
                    <tbody>
                        @if ($evaluaciones->isNotEmpty())
                            @foreach ($evaluaciones as $evaluacion)
                                <tr wire:key="{{ $evaluacion->id_evaluacion }}"
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end space-x-3">
                                            <!-- Ver -->
                                            <a href="{{ route('evaluacion/show', $evaluacion->id_evaluacion) }}" wire:navigate
                                            ...
                                            <!-- Editar -->
                                            <a href="{{ route('evaluacion/update', $evaluacion->id_evaluacion) }}" wire:navigate
                                            ...
                                            @if ($evaluacion->estatus == 1)
                                                <!-- Inhabilitar -->
                                                <button wire:click="confirmarInhabilitar({{ $evaluacion->id_evaluacion }})"
                                            ...
                                            @else
                                                <!-- Restaurar -->
                                                <button wire:click="confirmarRestaurar({{ $evaluacion->id_evaluacion }})"
                        ...
                        @else
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td colspan="3" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    {{ $busqueda ? 'No se encontraron evaluaciones' : 'No hay evaluaciones registradas' }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
                ...
                <x-table.delete-modal :show="$idInhabilitar !== null" title="Confirmar Inhabilitación"
                    message="¿Estás seguro de que deseas inhabilitar esta evaluación?"
                ...
                <x-table.delete-modal :show="$idRestaurar !== null" title="Confirmar Restauración"
                    message="¿Estás seguro de que deseas restaurar esta evaluación?"
