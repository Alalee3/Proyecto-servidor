        ...
        <form wire:submit.prevent="guardar" class="w-full space-y-6" novalidate>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="w-full">
                    ...
                    <x-text-input id="descripcion_evento" type="text" wire:model.live="form.descripcion_evento"
                        placeholder="Ej: Congreso Nacional" required :disabled="$form->is_especial" />
                ...
                <div class="w-full">
                    <x-input-label for="cantidad_repetible_evento_edit" :value="__('Límite de veces a repetir (2 a 5)')" />
                    <x-text-input id="cantidad_repetible_evento_edit" type="number" min="2" max="5" class="w-full"
                ...
                <x-toggle-switch id="is_fin_semana_evento_edit" :label="__('¿Puede asignarse en fines de semana?')"
                ...
                <x-toggle-switch id="is_semana_evento_edit" :label="__('¿Ocurre en semanas específicas?')"
                    model="form.is_semana_evento" :disabled="$deshabilitarSemanaEvento" required />
                                ...
                                <div wire:key="semana-lapso1-{{ $index }}">
                                    <div class="flex items-center gap-2">
                                        <x-text-input type="number" min="1" max="18" class="w-full"
                                            wire:model.live="form.semanas.{{ $index }}.semana" placeholder="Ej: 1"
                                            :disabled="$form->is_especial" required />
                                        @if($form->is_repetible && count($semanasLapso1) > 1 && !$form->is_especial)
                                ...
                                <div wire:key="semana-lapso2-{{ $index }}">
                                    <div class="flex items-center gap-2">
                                        <x-text-input type="number" min="1" max="18" class="w-full"
                                            wire:model.live="form.semanas.{{ $index }}.semana" placeholder="Ej: 1"
                                            :disabled="$form->is_especial" required />
                                        @if($form->is_repetible && count($semanasLapso2) > 1 && !$form->is_especial)
                                            <button type="button" wire:click="removerSemana({{ $index }})"
                                                class="text-red-500 hover:text-red-700 p-2 bg-red-50 dark:bg-red-900/20 rounded-md transition-colors"
                                                title="Eliminar semana">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
