        ...
        <form wire:submit.prevent="guardar" class="w-full space-y-6" novalidate>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <x-toggle-switch id="is_independiente_edit" :label="__('¿Puede registrarse fuera de un semestre?')"
                    model="form.is_independiente" :disabled="$deshabilitarIndependienteLaborable" />

                <x-toggle-switch id="is_superponible_edit" :label="__('¿Puede solaparse con otros eventos? (Superponible)')" model="form.is_superponible" :disabled="$deshabilitarSuperponible" />
                ...
                @if($form->is_especial && $form->especial_evento == '1')
                    <div class="w-full">
                        ...
                        <x-text-input id="cantidad_dias_evento" type="number" min="1" max="365" class="w-full"
                            wire:model.live="form.cantidad_dias_evento" placeholder="Ej: 15" :disabled="!$form->is_especial || $form->especial_evento != '1'" />
