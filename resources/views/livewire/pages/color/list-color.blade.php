        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 sm:rounded-lg">
            ...
            <div class="sogat-table-container">
                <div
                    ...
                    <x-table.search-input model="busqueda" placeholder="Buscar color..." debounce="300ms" />
                </div>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3 font-medium text-gray-900 dark:text-white">Código (Color)</th>
                            @can('cambiar-estatus-color')
                    ...
                    <tbody>
                        @if ($colores->isNotEmpty())
                            @foreach ($colores as $color)
                                <tr wire:key="{{ $color->id_color }}"
                                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-4 py-4 text-gray-900 dark:text-white break-words">{{ $color->nombre_color }}</td>
                                    <td class="px-4 py-4 text-gray-900 dark:text-white">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-sm font-mono">{{ strtoupper($color->codigo_color) }}</span>
                                        </div>
                                    </td>
                                    @can('cambiar-estatus-color')
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end space-x-3">
                                            @can('ver-color')
                                                <a href="{{ route('color.show', $color->id_color) }}" wire:navigate
                                            ...
                                            @endcan
                                            @can('editar-color')
                                                <a href="{{ route('color.update', $color->id_color) }}" wire:navigate
                                            ...
                                            @can('cambiar-estatus-color')
                                                @if ($color->estatus == 1)
                                                    <button wire:click="confirmarInhabilitar({{ $color->id_color }})"
                                                ...
                                                @else
                                                    <button wire:click="confirmarRestaurar({{ $color->id_color }})"
                        ...
                        @else
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td colspan="{{ auth()->user()->can('cambiar-estatus-color') ? 4 : 3 }}"
                ...
                <x-table.delete-modal :show="$idInhabilitar !== null" title="Confirmar Inactivación"
                    message="¿Estás seguro de que deseas inactivar este color?"
                ...
                <x-table.delete-modal :show="$idRestaurar !== null" title="Confirmar Activación"
                    message="¿Estás seguro de que deseas activar este color?"
