        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 sm:rounded-lg">
            ...
            <div class="sogat-table-container">
                ...
                <div
                    ...
                    <x-table.search-input model="busqueda" placeholder="Buscar técnica..." debounce="300ms" />
                </div>
                            ...
                            @foreach ($tecnicas as $tecnica)
                                <tr wire:key="{{ $tecnica->id_tecnica }}"
                                    ...
                                    <td class="px-4 py-4 text-right">
                                        <span class="{{ $tecnica->estatus == 1 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }} 
                                                                                    text-xs font-medium px-2.5 py-0.5 rounded">
                                            {{ $tecnica->estatus == 1 ? 'Activo' : 'Inactivo' }}
                                        </span>
                                    </td>
                                    <!-- Acciones -->
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end space-x-3">
                                            <!-- Ver -->
                                            @can('ver-tecnica')
                                                <a href="{{ route('tecnica/show', $tecnica->id_tecnica) }}" wire:navigate
                                            ...
                                            <!-- Editar -->
                                            @can('editar-tecnica')
                                                <a href="{{ route('tecnica/update', $tecnica->id_tecnica) }}" wire:navigate
                                            ...
                                            @can('editar-tecnica')
                                                @if ($tecnica->estatus == 1)
                                                    <!-- Inactivar -->
                                                    <button wire:click="confirmarInhabilitar({{ $tecnica->id_tecnica }})"
                                                ...
                                                @else
                                                    <!-- Activar -->
                                                    <button wire:click="confirmarRestaurar({{ $tecnica->id_tecnica }})"
                                                        class="flex items-center gap-1 bg-green-50 text-green-600 dark:bg-green-900 dark:text-green-200 hover:bg-green-100 dark:hover:bg-red-800 text-xs font-medium px-2.5 py-0.5 rounded text-left">
                        ...
                        @else
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td colspan="3" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                    {{ $busqueda ? 'No se encontraron técnicas' : 'No hay técnicas registradas' }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
                ...
                <x-table.delete-modal :show="$idInhabilitar !== null" title="Confirmar Inactivación"
                    message="¿Estás seguro de que deseas inactivar esta técnica de evaluación?"
                ...
                <x-table.delete-modal :show="$idRestaurar !== null" title="Confirmar Activación"
                    message="¿Estás seguro de que deseas activar esta técnica de evaluación?"
