...
<div {{ $attributes->merge(['class' => 'w-full relative']) }}
     x-data="{
        open: false,
        search: @entangle($wireModel),
        options: {{ $options->map(fn($o) => ['text' => $o->{$textField}, 'code' => $o->{$codeField}])->toJson() }},
        filteredOptions: [],
        highlightedIndex: -1,
        ...
        filterOptions() {
            if (!this.search) {
                ...
                this.filteredOptions = this.options.filter(o => o.text.toLowerCase().includes(s));
            }
        },
...
>
    ...
    <div class="flex items-center gap-1 w-full relative">
        {{-- Color swatch preview dentro del input --}}
        <template x-if="selectedColor">
            <span
                class="absolute left-2.5 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full border-2 border-gray-300 dark:border-gray-600 shadow-sm z-10 transition-all duration-200"
    ...
    >
        <template x-for="(option, index) in filteredOptions" :key="index">
            <div
                ...
                class="px-4 py-2.5 text-sm cursor-pointer transition-colors duration-150 border-b border-gray-100 dark:border-gray-700 last:border-0 flex items-center gap-3"
            >
                <span
                    class="inline-block w-5 h-5 rounded-full border border-gray-300 dark:border-gray-600 flex-shrink-0 shadow-sm"
